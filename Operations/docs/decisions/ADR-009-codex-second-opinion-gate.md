...
## This hub's own scope

This ADR records that the decision was made and implemented in
`tlelosa-claude-config`, not that `codex-gate` is installed or used on this
(Operations) machine. Per the decision above, rollout here is explicitly
**deferred** until Fan Movement IT confirms OpenAI egress from the work
machine — separate from, and not covered by, the 2026-07-21 clearance for
the personal Anthropic account and Context7. Root `CLAUDE.md`/`CORE.md` are
unchanged by this ADR; `/codex-review` does not exist as a usable command
in any Operations-hub session until that clearance lands and a follow-up
task installs the plugin here.
