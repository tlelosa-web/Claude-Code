...
def import_items_from_csv_skip_quantities(csv_path):
    """
    Import items from CSV but DO NOT update quantities.
    Only updates: description, category, costs, prices, and active status.
    Preserves existing stock quantities in the system.
    """
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"CSV file not found at: {csv_path}")
