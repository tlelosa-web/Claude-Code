...
- [x] **Fix Ollama generation-latency false-positive risk** (mitigation items #1+#2 from the latency issue): `src/research/ollama_client.py` `READ_TIMEOUT` bumped 60s → 120s, and `"keep_alive": "30m"` added to the `/api/generate` payload so gaps between leads in a real batch don't force a repeat cold-load via Ollama's default 5-minute idle-unload. 3 new unit tests (`READ_TIMEOUT` value, `timeout` tuple passed to `requests.post`, `keep_alive` in payload). Commit `3ec16cd`, **156 tests passing**, zero regressions. Item #3 (warm-up call at start of `run-all`) stays deferred — see Known Issues.

---

## Known Issues

- [ ] **(Deferred until batch volume justifies it)** A warm-up call at the start of `run-all` to pay the Ollama cold-load cost once, predictably, before the first real lead's timed request. This was mitigation item #3 for the latency issue below; #1 and #2 are shipped (see Completed). Not needed yet at current batch sizes — revisit if `OllamaError` timeouts recur even with the 120s ceiling + `keep_alive`.

## Closed — on hold pending affordability

- OpenRouter account out of credits (HTTP 402 on `asset_gen`, since 2026-07-04). Closed,
  not an active task — revisit once affordability improves. `research`'s half of this was
  already resolved independently by Build Queue B/ADR-004 (moved to local Ollama). The
  planned resolution for the remaining `asset_gen` half is Build Queue A (headless Claude
  Code migration), which doesn't depend on OpenRouter credits at all.
